...
document.getElementById('submit')
    .addEventListener('click', () => {
        ...
        //         })
        //     );
        //     outerPanel.translateY(Math.cos(thetaOffset + i / sides * Math.PI) * distance*1.01)
        //     outerPanel.translateX(Math.sin(thetaOffset + i / sides * Math.PI) * distance*1.01)
        //     outerPanel.translateZ(-4.5 / 2 - 0.02);
        ...
        for (let i = 0; i < 2; i++) {
            ...
            panel.translateX(Math.sin(thetaOffset + angles[i]) * 4.54 / 2)
            panel.translateZ(-4.5 / 2 - 0.02);
            panel.rotateZ(thetaOffset + angles[i] + Math.PI / 2)
            scene.add(panel);

            const outerPanel = new Mesh(
                new BoxGeometry(4.5 + 0.02, 0.002, 4.5),
                new MeshStandardMaterial({
                    color: '#FF0000',
                    roughness: 0.4,
                    metalness: 0.5
            ...
            outerPanel.translateY((-0.5) * 0.99)
            outerPanel.translateX(Math.sin(thetaOffset + angles[i]) * 4.54 / 2 * 1.02)
            outerPanel.translateZ(-4.5 / 2 - 0.02);
            outerPanel.rotateZ(thetaOffset + angles[i] + Math.PI / 2)
            scene.add(outerPanel);
        }
        ...
        function startRenderer(scene) {
            ...
            // camera.position.set(0, 0, -length / 1);
            // camera.lookAt(0, 0, 0);
